...
//   axios.post("http://127.0.0.1:8000/api/patients", values)
//     .then(() => {
...
const [selectedPatientId, setSelectedPatientId] = useState(null);
  ...
  const handleModifierFormSubmit = (values) => {
    ...
    axios.put(`http://127.0.0.1:8000/api/patients/${selectedPatientId}`, values)
      .then(() => {
        // Update successful, perform any necessary logic or redirect
...
//delete patient
  const handleDelete = (key) => {
    setLoading(true);
    axios
      .delete(`http://127.0.0.1:8000/api/patients/${key}`)
      .then(() => {
        ...
        axios
          .get(`http://127.0.0.1:8000/api/patients?page=${currentPage}&limit=${pageSize}`)
          .then((response) => {
  ...
  const columns = [
    ...
    { title: "sexe", dataIndex: "sexe", key: "sexe", width: '30%',
     },
